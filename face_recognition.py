...
class Face_Recognition:
    ...
    def __init__(self,root):
        ...
        # first header image  
        img=Image.open(r"C:\Users\Admin\Desktop\project2\Images_GUI\banner.jpg")
    ...
    def mark_attendance(self, i, r, n):
        ...
        with open("attendance.csv", "r+") as f:
            ...
            student_ids = [line.split(",")[0].strip() for line in myDatalist]
            

            if i not in student_ids:  # Check if the student ID is already present
    ...
    #================face recognition==================
    def face_recog(self):
        def draw_boundray(img,classifier,scaleFactor,minNeighbors,color,text,clf,marked_students):
            gray_image=cv2.cvtColor(img
            
            ,cv2.COLOR_BGR2GRAY)
            featuers=classifier.detectMultiScale(gray_image,scaleFactor = 1.2 ,minNeighbors=5)
            ...
            for (x,y,w,h) in featuers:
                ...
                n = cursor.fetchone()
                n = str(n[0]) if n else "" # Retrieve the first element and convert it to a string

                cursor.execute('select Roll_No from student where Student_ID='+str(id))
                r = cursor.fetchone()
                r = str(r[0]) if r else ""  # Retrieve the first element and convert it to a string

                cursor.execute('select Student_ID from student where Student_ID='+str(id))
                i = cursor.fetchone()
                i = str(i[0]) if i else "" # Retrieve the first element and convert it to a string
 
 
                if confidence > 77:
                    if i not in marked_students:  # Check if attendance has been marked for this student
                        self.mark_attendance(i, r, n)
                        marked_students.append(i)  # Add the student ID to the list of marked students
        ...
        #==========
        def recognize(img,clf,faceCascade,marked_students):
            coord=draw_boundray(img,faceCascade,1.1,10,(255,25,255),"Face",clf,marked_students)
            return img
        ...
        while True:
            ...
            img=recognize(img,clf,faceCascade,marked_students)
